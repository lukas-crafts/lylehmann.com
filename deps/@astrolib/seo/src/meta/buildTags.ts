...
const buildOpenGraphMediaTags = (
  ...
  return media.reduce((tags: string[], medium, index) => {
...
const buildTags = (config: BuildTagsParams) => {
  ...
  if (config.openGraph?.description || config.description) {
    tagsToRender.push(
      `<meta property="og:description" content="${escape((config.openGraph?.description || config.description) as string)}" />`
    );
  }
  ...
  if (config.additionalMetaTags && config.additionalMetaTags.length > 0) {
    config.additionalMetaTags.forEach((tag) => {
      const tagContent = Object.entries(tag)
        .map(([key, value]) => `${key}="${value}"`)
        .join(" ");
      tagsToRender.push(
        `<meta key="meta:${tag.keyOverride ?? tag.name ?? tag.property ?? tag.httpEquiv}" ${tagContent} />`
      );
    });
  }
  ...
  if (config.additionalLinkTags?.length) {
    config.additionalLinkTags.forEach((tag) => {
      const tagContent = Object.entries(tag)
        ...
        .join(" ");
      tagsToRender.push(
        `<link key="link${tag.keyOverride ?? tag.href}${tag.rel}" ${tagContent} />`
      );
    });
  }
