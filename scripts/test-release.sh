...
if pnpm exec semantic-release --dry-run --no-ci 2>&1 | tee /tmp/semantic-release-test.log; then
    EXIT_CODE=0
else
    EXIT_CODE=$?
fi
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    if grep -q "Published release" /tmp/semantic-release-test.log; then
        echo -e "${GREEN}✅ A new release would be created${NC}"
    elif grep -q "therefore a new version won't be published" /tmp/semantic-release-test.log; then
        echo -e "${YELLOW}ℹ️  No release would be created (branch not configured for releases)${NC}"
    elif grep -q "There are no relevant changes" /tmp/semantic-release-test.log; then
    ...
    if grep -q "ENOGHTOKEN" /tmp/semantic-release-test.log; then
        ...
        echo -e "${RED}Check /tmp/semantic-release-test.log for details${NC}"
        exit 1
    fi
fi
...
echo "  - Full test logs available in /tmp/semantic-release-test.log"
echo ""
